...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (address && userId) {
            ...
            LAYER.checkRequiredAddressFields(address);
            for (let key of Object.keys(address)) {
                address[key] = LAYER.validateUserData(key,address[key]);
            }
            ...
            // Modify user's address. Returns: { id: <userId>, accessToken: <token> }
            let result = await LAYER.createOrModifyUser(address, userId);

            // Success. Read/Return user address confirmation.
            response = await LAYER.getUser(result.id, null, true, true, false); // Return just the user's address
        }
    } catch (err) {
        error = err.toString();
