...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (event && event.user) {
            ...
            let result = await LAYER.createOrModifyUser(userData, userId);
            if (result.id) {
                response.user = await LAYER.getUser(result.id, event.user.email, true, false, false)
            } else {
                throw "Error finding the User."
            } 
        } else {
            throw "Unable to modify user. Invalid data fields received.";
        }
    } catch (err) {
        error = err.toString();
